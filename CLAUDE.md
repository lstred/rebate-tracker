...
- **Restore no longer requires restart:** After cloud or JSON file restore, `SettingsView` emits `restore_complete` → `MainWindow._on_data_restored()` reloads accounts/structures/dashboard/settings in-place. The restart dialog is gone.
